...
This Git repository is the release-assets repository, so it contains the Lua
modules and shader but not the full C++ checkout. The `src/client/*` files
listed above must be copied from the synchronized full OTClient source on the
Windows development machine. A client binary built without those C++ hooks
will load the module but cannot recolor map effects or missiles.
