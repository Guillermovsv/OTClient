...
function controllerCyclopedia:onInit()
    ...
    ProtocolGame.registerExtendedJSONOpcode(SPELL_DAMAGE_OPCODE, Cyclopedia.onParseSpellDamage)
end

function Cyclopedia.onParseSpellDamage(protocol, opcode, data)
    if type(data) ~= 'table' or data.action ~= 'spellDamage' then
        return
    end

    Spells.setDamageInfo(data.spells)

    -- the archives tab may already be open with a spell selected
    if Cyclopedia.refreshMagicalArchivesDamage then
        Cyclopedia.refreshMagicalArchivesDamage()
    end
end
...
function controllerCyclopedia:onGameEnd()
    ...
    -- damage is resolved for the character that just logged out
    Spells.clearDamageInfo()
...
function controllerCyclopedia:onTerminate()
    ProtocolGame.unregisterExtendedJSONOpcode(SPELL_DAMAGE_OPCODE)
