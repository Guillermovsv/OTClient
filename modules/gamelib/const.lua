...
-- Delyriumz server RSA public modulus (1024-bit, exponent 65537).
-- The DelyriumzOT server is deployed with the matching private key (key.pem),
-- so the stock OTSERV_RSA cannot encrypt the game-world session key for it.
-- HTTP login on :8088 carries no RSA, which is why the character list works
-- with the wrong key but entering the game silently fails.
-- Public modulus only -- never place the private key in the client.
DELYRIUMZ_RSA = '1474023717157539214295195504834487236408630681487027638969628434' ..
    '8234207885674572937093764207251004775103234830781841735510821753' ..
    '2198826252808843059024559538208703618344629920277587403090031960' ..
    '5609740903270446563286813464245658218841637037072952995523557628' ..
    '63069162604453589836326094298743504194909151257423241'
