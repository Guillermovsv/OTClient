...
-- These must match the server's CipbiaSkills_t enum (canary-src/src/utils/utils_definitions.hpp),
-- not a plain sequential 1-8 list - the server sends these exact (non-sequential, gapped) byte
-- values in the CLIENT_EVENT_TYPE_SKILL banner packet. Using sequential IDs here made every
-- skill above Fist Fighting show an adjacent skill's name (e.g. Shield showed as Fishing, Axe
-- showed as Distance).
local SkillId = {
    Magic = 1,
    Shielding = 6,
