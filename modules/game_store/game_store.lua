...
-- HTTP.images only caches successes, so a missing remote image is re-requested
-- (and stalls for HTTP.timeout) every time it appears. Remember the failures too.
local failedImageUrls = {}

local function joinImageUrl(base, path)
    if base:sub(-1) == "/" then
        base = base:sub(1, -2)
    end
...
local function localImagePath(url)
    if url:sub(1, 1) ~= "/" then
        url = "/" .. url
    end
    return "/game_store/images" .. url
end

local function applyImage(widget, path, isIcon)
    if isIcon then
        widget:setIcon(path)
    else
...
local function applyImageError(widget, isIcon)
    if isIcon then
...
local function setImagenHttp(widget, url, isIcon)
    if not url or url == "" then
        return applyImageError(widget, isIcon)
    end

    -- local pack wins: no request, no per-image timeout
    local localPath = localImagePath(url)
    if g_resources.fileExists(localPath) then
        return applyImage(widget, localPath, isIcon)
    end

    local remote = GameStore.website.IMAGES_URL
    if not remote or remote == "" then
        return applyImageError(widget, isIcon)
    end

    local fullUrl = joinImageUrl(remote, url)
    if failedImageUrls[fullUrl] then
        return applyImageError(widget, isIcon)
    end
    ...
    HTTP.downloadImage(fullUrl, function(path, err)
        ...
        if err then
            if not failedImageUrls[fullUrl] then
                failedImageUrls[fullUrl] = true
                g_logger.warning("HTTP error: " .. err .. " - " .. fullUrl)
            end
            return applyImageError(widget, isIcon)
        end
        applyImage(widget, path, isIcon)
    end)
end
...
function onParseStoreCreateHome(offer)
    ...
    if offer.banners and #offer.banners > 0 then
        currentIndex = math.random(1, #offer.banners)
        setImagenHttp(controllerShop.ui.HomePanel.HomeImagen, offer.banners[currentIndex].image, false)
    else
        currentIndex = 1
        setImagenHttp(controllerShop.ui.HomePanel.HomeImagen, "", false)
    end
    enableAllButtons()
...
function onParseStoreGetCategories(buttons)
    ...
    -- "VIP Shop" is what the server names this category when vipSystemEnabled is
    -- on (data/modules/scripts/gamestore/catalog/premium_time.lua); we sell VIP
    -- rather than Premium, so it needs the same sort slot or it falls to the end.
    local orderedCategoryNames = {"Home", "VIP Shop", "Premium Time", "Consumables", "Cosmetics", "Houses", "Boosts",
                                "Extras", "Tournament"}
...
function changeImagenHome(direction)
    ...
    setImagenHttp(controllerShop.ui.HomePanel.HomeImagen, currentBanner.image, false)
end
