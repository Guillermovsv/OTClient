...
Services = {
    ...
    status = "https://delyriumzot.com/status.php", --./client_entergame | ./client_topmenu
...
if ENABLE_SERVERS then
    ...
    Servers_init = {
        ...
        -- OTServer (game.delyriumzot.com)
        ---
        -- Configuration for our Canary server.
        -- @class table
        -- @name otserver
        -- @field port Game protocol port
        -- @field protocol Client protocol version (15.24/15.25 line)
...
g_app.setName("DelyriumzOT Server");
...
-- Not using ensureModuleLoaded here (it g_logger.fatal()s the whole client
-- on any failure) - shaders are a visual nice-to-have, not something a
-- crashed-out player can do anything about. If it fails to load for any
-- reason (bad GPU driver, corrupted install, etc.) just log it and keep
-- going without shader effects instead of fatal-crashing the client.
do
    local shadersModule = g_modules.getModule('game_shaders')
    if not shadersModule or not shadersModule:load() then
        g_logger.error('game_shaders module failed to load - continuing without shader effects.')
    end
end
